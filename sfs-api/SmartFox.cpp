...
void SmartFox::Connect(string host, long int port)
{
	...
	bitSwarm->Connect(endpointAddress, port);
}
...
boost::shared_ptr<string> SmartFox::GetIPAddress(string host, long int port)
{
	...
	tcp::resolver::query query(forceIPv6 == false ? tcp::v4() : tcp::v6(), host, *portAsString);
...
void SmartFox::InitUDP(string udpHost, unsigned short int udpPort)
{
	...
	try {
		...
		tcp::resolver::query query(forceIPv6 == false ? tcp::v4() : tcp::v6(), udpHost, *portAsString);
